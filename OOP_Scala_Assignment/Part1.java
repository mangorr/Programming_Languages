...
interface SetOf<T extends LessThanOrEqualTo<T>> extends LessThanOrEqualTo<SetOf<T>>, Iterable<T>{
    ...
    public <E extends SetOf<T>> boolean subset(E x);

    public <E extends SetOf<T>> SetOf<T> union(E x);

    public <E extends SetOf<T>> SetOf<T> intersection(E x);

    public <E extends SetOf<T>> SetOf<T> difference(E x);
}
...
class ASet<T extends LessThanOrEqualTo<T>> implements SetOf<T>, LessThanOrEqualTo<SetOf<T>> {
    ...
    @Override
    public <E extends SetOf<T>> SetOf<T> union(E x) {
        SetOf<T> result = new ASet<T>();
        for (T s : list) {
            result.add(s);
        }
        for (T s : x.difference(this)) {
            result.add(s);
        }
    ...
    public boolean lessThan(SetOf<T> t) {
        if (this.subset(t) && !t.subset(this)) return true;
        else return false;
    }
...
class Part1 {
    ...
    static <T extends LessThanOrEqualTo<T>> SetOf<SetOf<T>> powerSet(SetOf<T> s){
        SetOf<SetOf<T>> ans = new ASet<SetOf<T>>();
        ...
        if (s.empty()) {
            ...
            T t = s.get();
            SetOf<T> part = new ASet<T>();
            part.add(t);

            SetOf<T> diff_part = s.difference(part);
            SetOf<SetOf<T>> union_part = powerSet(diff_part);
            ans = ans.union(union_part);

            for(SetOf<T> sub : union_part){
                ans.add(sub.union(part));
            }
            return ans;
        }
    }
