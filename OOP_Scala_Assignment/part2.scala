...
case class Leaf[+T](v:T) extends Tree[T] {
  override def map[V](f:T=>V):Leaf[V] = Leaf(f(v))
...
case class Node[+T](v:T, children: List[Tree[T]]) extends Tree[T] {
  override def map[V](f:T=>V):Tree[V] = Node(f(v), children.map(x=>x.map(f)))
  override def toString(): String = (s"Node($v," + children.toString() + ")")
}  // Roughly 4 lines
...
class A(x:Int) {
  def value():Int = x
  def +(other:A) = new A(other.value() + value())
...
object Part2 {
  ...
  def breadthFirst[T](tr: Tree[T]): List[T] = {
    def breadth (listTree: List[Tree[T]], store:List[T]) : List[T] = listTree match{
      case List() => store
      case (Leaf(v)::res) => breadth(res, store:::List(v))
      case (Node(v, children)::res) => breadth(res:::children, store:::List(v))
      case _ => List()
    }
  ...
  def reduce[T,V](f:(T, V)=>V, b:V, L:List[T]):V = L match {
    case List()=>b
    case (head::res)=>f(head, reduce(f, b, res))
  } // Roughly 4 lines
  ...
  def sumTreeA(t: Tree[A]):A = t match {
    case Leaf(v)=>v
    case Node(v, children)=>def f(x:A,y:A):A = x + y
                            v + reduce[A,A](f:(A,A)=>A, new A(0), children.map(sumTreeA))
  } // Roughly 4 lines
